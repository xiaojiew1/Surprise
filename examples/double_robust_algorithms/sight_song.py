...
from config import song_lr_count
...
lr_all_opt = sorted(list(lr_all_opt))[:song_lr_count]
reg_all_opt = sorted(list(reg_all_opt))
print(lr_all_opt)
print(reg_all_opt)
lr_part_opt = [lr_all_opt[i] for i in [0, 1,]]
reg_part_opt = [reg_all_opt[i] for i in [0, 1, 2, 3,]]
...
for lr_all, reg_all in itertools.product(lr_part_opt, reg_part_opt):
  ...
  indexes = np.arange(150, len(errors))
