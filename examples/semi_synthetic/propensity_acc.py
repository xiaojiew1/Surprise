from config import gamma_dir, data_dir, figure_dir
...
def collect_gamma(alpha, betas, gamma, dataset, recom_list, risk):
  d_betas, sd_betas = betas
  n_users, n_items, cmpl_rates, cmpl_cnt = dataset
  risk_name, risk = risk

  out_file = path.join(gamma_dir, '%s_%.1f_%.1f.p' % (risk_name, alpha, gamma))

  k = utils.solve_k(alpha, n_users, n_items, cmpl_cnt)
  print('alpha=%.2f gamma=%.2f k=%.4f' % (alpha, gamma, k))
  print('\n' + '#'*n_hashtag + '\n')
  ...
  for recom in recom_list:
    ...
    res = utils.evaluate_est(recom, dataset, cmpl_props, betas, risk, gamma=gamma)
  ...
  if path.isfile(out_file):
    ...
    return
  data = {
    'alpha': alpha, 'gamma': gamma, 'n': n_risk_rmse,
    'p': p_risk_rmse, 'sp': sp_risk_rmse,
    'd': d_risk_rmse, 'sd': sd_risk_rmse,
  }
  pickle.dump(data, open(out_file, 'wb'))

out_dir = path.join(data_dir, 'imputation_acc')
